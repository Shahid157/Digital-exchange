...
function CryptoSavings(props: GenericNavigation) {
  ...
  const entries = useEntries(StakingTypes.CryptoBank, stakingEntries.data);
  ...
  const staking = stakingsQuery.data?.find(
    (item) => item.type === StakingTypes.CryptoBank
  );
